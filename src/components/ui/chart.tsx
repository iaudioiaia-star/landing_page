...
const VALID_COLOR_REGEX = /^(#[0-9A-Fa-f]{3,8}|rgb\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*\)|rgba\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*,\s*[\d.]+\s*\)|hsl\(\s*[\d.]+\s*,?\s*[\d.]+%?\s*,?\s*[\d.]+%?\s*\)|hsla\(\s*[\d.]+\s*,\s*[\d.]+%\s*,\s*[\d.]+%\s*,\s*[\d.]+\s*\)|[a-zA-Z]+)$/;

// Sanitize color values to prevent CSS injection
const sanitizeColor = (color: string | undefined): string | null => {
  if (!color) return null;
  // Remove any whitespace and check against valid pattern
  const trimmedColor = color.trim();
  if (!VALID_COLOR_REGEX.test(trimmedColor)) {
    return null;
  }
  // Additionally escape any characters that could break out of CSS context
  const escapedColor = trimmedColor.replace(/[<>"'`;{}]/g, '');
  return escapedColor;
};

// Sanitize CSS selector ID to prevent injection
...
${colorConfig
  .map(([key, itemConfig]) => {
    ...
    // Also sanitize the key to prevent injection via config keys
